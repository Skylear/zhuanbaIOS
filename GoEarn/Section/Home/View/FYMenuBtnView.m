...
-(id)initWithFrame:(CGRect)frame title:(NSString *)title imagestr:(NSString *)imagestr//形成视图和文字的view
{
    ...
    if(self)
    {        
        UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(frame.size.width/2-22, 10, 40, 40)];
        [imageView sd_setImageWithURL:[NSURL URLWithString:imagestr] placeholderImage:placeholder_list];
        [self addSubview:imageView];
        
        
        UILabel *titlelab = [[UILabel alloc]initWithFrame:CGRectMake(imageView.minX, 5+imageView.maxY, imageView.width, 20)];
        titlelab.text = title;
        titlelab.textAlignment = NSTextAlignmentCenter;
        titlelab.font = [UIFont systemFontOfSize:12];
        [self addSubview:titlelab];
