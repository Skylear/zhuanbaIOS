...
- (void)viewDidLoad {
    ...
    // 默认的外观，你可以在这个方法后重置
    self.oKButtonTitleColorNormal   = UIColorFromRGB(0xff4c61);
    self.oKButtonTitleColorDisabled =UIColorFromRGB(0xcdcdcd);
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    _originStatusBarStyle = UIStatusBarStyleDefault;
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    ...
    if (self) {
        ...
        self.barItemTextColor = UIColorFromRGB(0x4c4c4c);
...
- (void)viewDidLoad {
    ...
    self.navigationItem.backBarButtonItem = [[UIBarButtonItem alloc]initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:nil action:nil];
}
