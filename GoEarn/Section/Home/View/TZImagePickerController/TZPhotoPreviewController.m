...
- (void)configCustomNaviBar {
    ...
    _backButton = [[UIButton alloc] initWithFrame:CGRectMake(10, 18, 44, 44)];
    [_backButton setImage:[UIImage imageNamed:@"return_btn"] forState:UIControlStateNormal];
    _backButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
    [_backButton setTitleColor:UIColorFromRGB(0xcdcdcd) forState:UIControlStateNormal];
    [_backButton addTarget:self action:@selector(back) forControlEvents:UIControlEventTouchUpInside];
    
    self.navTitleLabel = [[UILabel alloc]initWithFrame:CGRectMake(50, 18, SCREEN_WIDTH -100, 42)];
    self.navTitleLabel.textAlignment = NSTextAlignmentCenter;
    self.navTitleLabel.textColor = [UIColor blackColor];
    self.navTitleLabel.text = [NSString stringWithFormat:@"%ld/%ld",self.currentIndex +1,self.models.count];
    self.navTitleLabel.font = Font(17);
    [_naviBar addSubview:self.navTitleLabel];
    
    _selectButton = [[UIButton alloc] initWithFrame:CGRectMake(self.view.tz_width - 54, 18, 42, 42)];
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.scrollLeftOrRight ==NO) {
        self.navTitleLabel.text = [NSString stringWithFormat:@"%ld/%ld",station+1,self.models.count];
    }else
    {
        self.navTitleLabel.text = [NSString stringWithFormat:@"%ld/%ld",station-1,self.models.count];
    }
