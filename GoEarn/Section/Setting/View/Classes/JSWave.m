...
- (void)initData{
    ...
    gradientLayer.colors = @[(id)UIColorFromRGB(0xff4c61).CGColor,
                             (id)UIColorFromRGB(0xfe6a4e).CGColor];
    ...
    UIView *fontView = [[UIView alloc]initWithFrame:self.bounds];
    ...
    self.waveHeight = 10;
    self.realWaveColor = [[UIColor whiteColor] colorWithAlphaComponent:0.04];
    self.maskWaveColor = [[UIColor whiteColor] colorWithAlphaComponent:0.045];
    
    [fontView.layer addSublayer:self.realWaveLayer];
    [fontView.layer addSublayer:self.maskWaveLayer];
...
- (void)wave{
    ...
    for (CGFloat x = 0.f; x <= width ; x++) {
        y = height * sinf(0.01 * self.waveCurvature * x + self.offset * 0.045) - 60;
