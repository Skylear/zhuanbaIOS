...
- (void)setProgress:(float)progress
{
    ...
    if (progress > _progress || progress == 0) {
        _progress = progress;
        [self performSelector:@selector(upDateProgressOnline) withObject:self afterDelay:1.5];
        if (_progressBlock) {
            _progressBlock(progress);
        }
    }
}
-(void)upDateProgressOnline
{
    if ([_progressDelegate respondsToSelector:@selector(webViewProgress:updateProgress:)]) {
        [_progressDelegate webViewProgress:self updateProgress:_progress];
    }
}
- (void)reset
{
