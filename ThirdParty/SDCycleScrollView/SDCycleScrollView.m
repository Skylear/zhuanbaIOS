...
- (void)initialization
{
    ...
    _titleLabelTextFont= [UIFont systemFontOfSize:ad_Font];
    _titleLabelBackgroundColor = [UIColor clearColor];
    _titleLabelHeight = 25*SCREEN_POINT;
    _autoScroll = YES;
    _infiniteLoop = YES;
...
- (void)setupMainView
{
    ...
    _bgView = [[UIView alloc]initWithFrame: CGRectMake(0, self.maxY-25*SCREEN_POINT, SCREEN_WIDTH, 25*SCREEN_POINT)];
    ...
    self.titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(10, self.maxY-25*SCREEN_POINT, SCREEN_WIDTH-70, 25*SCREEN_POINT)];
    self.titleLabel.backgroundColor = self.titleLabelBackgroundColor;
    self.titleLabel.font=self.titleLabelTextFont;
...
- (void)layoutSubviews
{
    ...
    if (self.pageControlAliment == SDCycleScrollViewPageContolAlimentRight) {
        x = self.mainView.sd_width - size.width - 10*SCREEN_POINT;
    }
    CGFloat y = self.mainView.sd_height - size.height - 10*SCREEN_POINT;
...
//                                 progress:nil                                transform:^UIImage *(UIImage *image, NSURL *url) {
//                                    image = [image yy_imageByResizeToSize:CGSizeMake(SCREEN_WIDTH, SCREEN_WIDTH/2) contentMode:UIViewContentModeCenter];
...
//                                   }
//                               }];
            NSData *data = [ProTool findImgInDish:imagePath];
            UIImage *img =[UIImage imageWithData:data];
            cell.imageView.image = [ProTool handleImage:img withSize:CGSizeMake(SCREEN_WIDTH, scalW_2)];
            ...
            cell.imageView.image =  [ProTool handleImage:image withSize:CGSizeMake(SCREEN_WIDTH, scalW_2)];

        }
    } else if (!self.onlyDisplayText && [imagePath isKindOfClass:[UIImage class]]) {

        cell.imageView.image =  [ProTool handleImage:(UIImage *)imagePath withSize:CGSizeMake(SCREEN_WIDTH, scalW_2)];
    }
    
    if (_titlesGroup.count && itemIndex < _titlesGroup.count&&itemIndex==0) {
        self.titleLabel.text =_titlesGroup[0];
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
    ...
    int itemIndex = [self currentIndex];
    int indexOnPageControl = itemIndex % self.imagePathsGroup.count;
    self.titleLabel.text =_titlesGroup[indexOnPageControl];
