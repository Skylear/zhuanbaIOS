...
- (UIBarButtonItem *)backBtn{
    if (!_backBtn) {
        ...
        [back_btn setTitleColor:UIColorFromRGB(0x666666) forState:UIControlStateNormal];
...
- (UIButton *)doneBtn{
    if (!_doneBtn) {
        ...
        [_doneBtn setTitleColor:UIColorFromRGB(0xcdcdcd) forState:UIControlStateNormal];
    }
    return _doneBtn;
}
...
- (UIButton *)previewBtn{
    if (!_previewBtn) {
        ...
        _previewBtn.titleLabel.font = [UIFont systemFontOfSize:16.0f];
        [_previewBtn setTitleColor:UIColorFromRGB(0xcdcdcd) forState:UIControlStateNormal];
...
- (UILabel *)totalRound{
    if (!_totalRound) {
        ...
        if (self.roundColor == nil) {
            _totalRound.backgroundColor = UIColorFromRGB(0xcdcdcd);
        }else{
            _totalRound.backgroundColor = UIColorFromRGB(0xcdcdcd);
        }
        _totalRound.layer.masksToBounds = YES;
        _totalRound.textAlignment = NSTextAlignmentCenter;
        _totalRound.textColor = UIColorFromRGB(0xededed);
        _totalRound.text = @"0";
...
- (void)selectPhotoAtIndex:(NSInteger)index
{
    ...
    if (photo != nil) {
        if (photo.isSelect == NO) {
                ...
                if ([self.datas CheckIsiCloudAsset:photo.asset] == YES) {
                   ...
                   photo.isSelect = YES;
                    ...
                    [self.selectArray insertObject:[self.photoArray objectAtIndex:index] atIndex:self.selectArray.count];
                        [self.previewBtn setTitleColor:UIColorFromRGB(0x333333) forState:UIControlStateNormal];
                        self.totalRound.backgroundColor = UIColorFromRGB(0xff4c61);
                        self.totalRound.textColor = [UIColor whiteColor];
                        [self.doneBtn setTitleColor:UIColorFromRGB(0xff4c61) forState:UIControlStateNormal];
            ...
            if (self.selectArray.count ==0) {
                [self.previewBtn setTitleColor:UIColorFromRGB(0xcdcdcd) forState:UIControlStateNormal];
                self.totalRound.backgroundColor = UIColorFromRGB(0xcdcdcd);
                self.totalRound.textColor = UIColorFromRGB(0xededed);
                [self.doneBtn setTitleColor:UIColorFromRGB(0xcdcdcd) forState:UIControlStateNormal];
            }
            [[ZZAlumAnimation sharedAnimation] roundAnimation:self.totalRound];
            self.totalRound.text = [NSString stringWithFormat:@"%lu",(unsigned long)self.selectArray.count];
...
{
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:[NSString stringWithFormat:Alert_Max_Selected,(long)photoNumOfMax] message:nil preferredStyle:UIAlertControllerStyleAlert];
    
    UIAlertAction *action1 = [UIAlertAction actionWithTitle:@"我知道了" style:UIAlertActionStyleCancel handler:^(UIAlertAction *action){
